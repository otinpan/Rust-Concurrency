...
fn main(){
    ...
    let async_block_future=AsyncBlockFuture::new(Duration::from_secs(7));

    let async_block_future=Some(boxed(async_block_future));

    let futures_async_block=vec![async_block_future];
    /*
    let fut = async {
        println!("start");
    ...
    と同じ
     */
    spawner.spawn("Task in async block future",futures_async_block);
