...
fn main(){
    ...
    let sleep_future_2sec=SleepFuture::new(Duration::from_secs(2));
    let sleep_future_5sec=SleepFuture::new(Duration::from_secs(5));

    let my_future=Some(boxed(my_future));
    let sleep_future_2sec=Some(boxed(sleep_future_2sec));
    let sleep_future_5sec=Some(boxed(sleep_future_5sec));

    let futures_2sec=vec![my_future,sleep_future_2sec];
    let futures_5sec=vec![sleep_future_5sec];
    spawner.spawn("2sec Sleeping",futures_2sec);
    spawner.spawn("5sec Sleeping",futures_5sec);
