...
fn main(){
    ...
    let sleep_future=SleepFuture::new(Duration::from_secs(2));


    let sleep_task =Arc::new(Task{
        name: String::from("SleepTask"),
        futures: Mutex::new(vec![Some(Box::pin(sleep_future)),Some(Box::pin(my_future))]),
